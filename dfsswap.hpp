...
#ifdef MAINNET
  #define DFSSWAP_CONTRACT name("defisswapcnt")
#else
  #define DFSSWAP_CONTRACT name("testdfsswap1")
#endif
...
namespace dfsswap {
  ...
  market get_market(extended_symbol coin, extended_symbol anchor, std::vector<std::string> args) {
    market_tlb markets(DFSSWAP_CONTRACT, DFSSWAP_CONTRACT.value);
    if (args.size() > 0) {
      uint64_t id = std::atoi(args[0].c_str());
      char* msg;
      sprintf(msg, "cannot find dfs swap market with id: %d", id);
    ...
    check(false, "dfs swap market with this ticker not found");
    return market();
  }
  ...
  weight_price fetch(extended_symbol coin, extended_symbol anchor, std::vector<std::string> args) {
    ...
    if (anchor.get_contract() == m.contract0 && anchor.get_symbol() == m.sym0 && 
      ...
      check(false, "dfs swap market not match");
    }
